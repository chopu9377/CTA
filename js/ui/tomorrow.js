...
function bodyHTML(data, ctx, today, tomorrow) {
  ...
  return `${bonus ? `<div class="notice">내일 ${formatDuration(bonus)}은 보상 휴식이라 목표가 줄어 있어요 🎁</div>` : ""}
    ${rows.map((r) => `<div class="goal-row tmr-row${r.tag ? " maint" : ""}">
      ...
      <div class="goal-count">${r.amount}<small>${unit(r.goal)}</small></div></div>`).join("")}
    ...
    <p class="hint">${nextWeek ? "새 주의 첫날이라 지금 진도 기준 예상이에요. 오늘 더 공부하면 달라질 수 있어요." : "지금 기록 기준이에요. 오늘 더 하거나 되돌리면 바뀔 수 있어요."}</p>`;
}
...
export function tomorrowCardHTML(data, ctx, today, ui) {
  ...
  return `<div class="card tmr-card">
    ...
    ${open ? bodyHTML(data, ctx, today, tomorrow) : ""}
  </div>`;
}
